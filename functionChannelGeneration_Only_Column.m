function [HH,R_Vec,H_Vec,Omega,U_APR,U_UET] = functionChannelGeneration_Only_Column(channelGain,M,K,N,L,nbrOfRealizations)
%%=============================================================
%The file is used to generate the coupling matrix with only one column (case (c) in Fig.2) based channel of the paper:
%
...
for k = 1:K
    for m = 1:M
        ...
        Omega0 = sort(rand(L,1),'descend');
        Omega((m-1)*L+1:m*L,(k-1)*N+1) = channelGain(m,k)*Omega0/sum(Omega0(:))*L*N; % generate the coupling matrix randomly
